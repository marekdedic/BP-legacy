...
function relugprelugp(k,n;T=Float32)
  ...
  modelname=@sprintf("relugprelugp_%d_%d_%d_%d_%d_%d",k[1],k[2],k[3],k[4],n[1],n[2])
  return(trnmodel,tstmodel,modelname)
end
...
function getmodel(modeltype,k,n;T=Float32)
  if modeltype=="relumaxrelumax"
    return(relumaxrelumax(k,T=T))
  elseif modeltype=="relumeanrelumean"
    return(relumeanrelumean(k,T=T))
  elseif modeltype=="relugprelugp"
    return(relugprelugp(k,n,T=T))
  end
end